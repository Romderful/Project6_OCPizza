...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `oc_pizza`.`unavailable_products` (`stock_id` INT, `establishment_id` INT, `product_name` INT, `ingredient_name` INT, `unit` INT, `quantity_required` INT);
...
USE `oc_pizza`;
CREATE  OR REPLACE VIEW `client_informations` AS
    SELECT 
        `user`.`id` AS `user_id`,
...
USE `oc_pizza`;
CREATE  OR REPLACE VIEW `command_informations` AS
    SELECT 
        `command`.`id` AS `command_id`,
...
USE `oc_pizza`;
CREATE  OR REPLACE VIEW `employee_informations` AS
    SELECT 
        `user`.`id` AS `user_id`,
...
USE `oc_pizza`;
CREATE  OR REPLACE VIEW `establishment_informations` AS
    SELECT 
        `establishment`.`id` AS `establishment_id`,
...
USE `oc_pizza`;
CREATE  OR REPLACE VIEW `establishment_revenue` AS
    SELECT 
        `command`.`establishment_id` AS `establishment_id`,
...
USE `oc_pizza`;
CREATE  OR REPLACE VIEW `product_recipe` AS
    SELECT 
...
USE `oc_pizza`;
CREATE  OR REPLACE VIEW `unavailable_ingredients` AS
    SELECT 
...
USE `oc_pizza`;
CREATE  OR REPLACE VIEW `unavailable_products` AS
    SELECT 
        `stock`.`id` AS `stock_id`,
        `stock`.`establishment_id` AS `establishment_id`,
        `product`.`name` AS `product_name`,
        `ingredient`.`name` AS `ingredient_name`,
        `stock`.`unit` AS `unit`,
        `product_has_ingredient`.`quantity` AS `quantity_required`
    FROM
        (((`stock`
        JOIN `ingredient` ON ((`ingredient`.`id` = `stock`.`ingredient_id`)))
